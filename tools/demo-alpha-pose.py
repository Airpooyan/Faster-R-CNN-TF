...
def vis_detections(im, image_name, class_name, dets,xminarr,yminarr,xmaxarr,ymaxarr,results,score_file,index_file,num_boxes,map_file,thresh=0.4):
...
def demo(sess, net, image_name,xminarr,yminarr,xmaxarr,ymaxarr,results,score_file,index_file,num_boxes,imagedir, mode,map_file):
    ...
    num_boxes = vis_detections(im, image_name, cls, dets,xminarr,yminarr,xmaxarr,ymaxarr,results,score_file,index_file,num_boxes,map_file,thresh=CONF_THRESH)
    if(dets.shape[0]!=0):
        index_file.write("{}\n".format(num_boxes))
    return num_boxes
...
if __name__ == '__main__':
    ...
    map_file=open("result_0521_pretrain.txt",'w')
 
    num_boxes = 0
    for im_name in tqdm(im_names):
        #print('Human detection for {}'.format(im_name))
        num_boxes=demo(sess, net, im_name, xminarr,yminarr,xmaxarr,ymaxarr,results,score_file,index_file,num_boxes,inputpath, mode,map_file)
