...
def im_to_blob(im,scale):
    blobs={}
    blobs['data'],im_scale_factors=_get_image_blob(im,scale)
    return blobs,im_scale_factors

def im_detect(sess, net, im,scales=[600,800,1000]):
  ...
  number_scale=len(scales)
  last_scores,last_boxes=[],[]
  for i in range(number_scale):
      if i ==0:
         last_scores,last_boxes=finalscores[i],finalpredboxes[i]
      else:
         last_scores,last_boxes=np.concatenate((last_scores,finalscores[i])),np.concatenate((last_boxes,finalpredboxes[i]))
         
  return last_scores,last_boxes
